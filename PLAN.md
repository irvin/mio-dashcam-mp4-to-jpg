...
- 與 EXIF **同一套 GPS** 同步一份（部分軟體只讀 XMP）。
- **單一真相來源**：先產生一份結構化 metadata 物件，再由同一份資料同時寫入 EXIF 與 XMP，避免雙寫漂移。
- Dublin Core：`dc:date`、`dc:description` 等利於管理。
- **非標準欄位**（完整 GSV、`$GSENSORD` 原文）：放 **UserComment**、**關鍵字**或 **sidecar `.json`**。
...
4. **截圖**：依 **3.1 節**之 `frame_index`（由 `t_base`）決定畫面，再以 ffmpeg 或 filter 批次輸出；檔名包含 UTC 與 frame index，例如 `2026-04-03T01-35-46Z_f0000.jpg`。
